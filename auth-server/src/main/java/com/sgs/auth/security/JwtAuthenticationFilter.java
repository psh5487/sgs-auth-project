    ...
    public void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterchain) throws IOException, ServletException {
        ...
        if (jwtToken != null) {
            ...
            if(jwtTokenProvider.isTokenAlive(jwtToken)) { // 1. Access Token 이 유효한 경우
                ...
                String refreshTokenInRedis = redisUtil.getData(userEmail);

                // 2-1. Refresh Token 이 유효할 경우 - Redis 에 존재 & (사용자가 갖고 있던 토큰의 UserPk 와 Redis 에 저장된 토큰의 UserPk가 같아야함)
                if(refreshTokenInRedis != null && userEmail.equals(jwtTokenProvider.getUserPk(refreshTokenInRedis))) {
                    log.info("Refresh token is valid");

                    // Access Token 재발급
                    jwtToken = jwtTokenProvider.createToken(userEmail, role, JwtTokenProvider.ACCESS_TOKEN_VALID_TIME);

                    // 유저 정보를 받아옴
                    Authentication authentication = jwtTokenProvider.getAuthenticationWithUserPk(userEmail);
